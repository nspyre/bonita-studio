...
public class TestAutomaticSwitchPerspective extends TestCase {
    ...
    public void testSwitchPerspectiveToForms() throws Exception {
        ...
        assertTrue("Wrong perspective when opening the form", new TestAsyncThread(10, 500) {
            ...
            public boolean isTestGreen() throws Exception {
                ...
                final RunnableWithResult<Boolean> runnable = new RunnableWithResult<Boolean>() {
                    ...
                    @Override
                    public void run() {
                        processPerspective = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().getPerspective().getId()
                                .equals(ProcessPerspective);
                    }
        ...
        final CreateFormCommand formCommand = new CreateFormCommand(pageflow, ProcessPackage.Literals.PAGE_FLOW__FORM, "testForm", "form to test perspectives",
                Collections.<WidgetMapping> emptyList(), editingDomain);
        ...
        assertTrue("Wrong perspective when closing the form", PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().getPerspective().getId()
                .equals(ProcessPerspective));
