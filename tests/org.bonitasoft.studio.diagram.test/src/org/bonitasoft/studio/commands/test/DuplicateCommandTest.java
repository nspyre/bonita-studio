...
public class DuplicateCommandTest extends SWTBotGefTestCase {
    ...
    private void baseTest(boolean sourceDirty) throws PartInitException {
        ...
        String processVersion = botEditor.getTitle().substring(botEditor.getTitle().lastIndexOf("(") + 1, botEditor.getTitle().lastIndexOf(")"))
                + " duplicated";
        ...
        for (Pool p : pools) {
            ...
            assertTrue("Actor mapping is missing in process configuraiton after duplicate for " + p.getName() + " (" + p.getVersion() + ")", !conf
                    .getActorMappings().getActorMapping().isEmpty());
        }
    }
