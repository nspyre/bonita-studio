    ...
    public void migrateBefore(Model model, Metamodel metamodel) throws MigrationException {
        for (Instance correlationInstance : model.getAllInstances("process.Correlation")) {
            ...
            for (Instance association : correlationAssociations) {
                ...
                if (correlationKey != null || correlationExpression != null) {
                    ...
                    if (correlationKey != null && !correlationKey.isEmpty()) {
                        ...
                        keyExpression = StringToExpressionConverter.createExpressionInstance(model, "", "", String.class.getName(),
                    ...
                    if (correlationExpression != null && !correlationExpression.isEmpty()) {
                        ...
                        if (ExpressionConstants.SCRIPT_TYPE.equals(valueExpression.get("type"))) {
                            ...
                            valueExpression.set("returnType",
                                    StringToExpressionConverter.getDataReturnType(((List<Instance>) valueExpression.get("referencedElements")).get(0)));
                        }
                    } else {
                        valueExpression = StringToExpressionConverter.createExpressionInstance(model, "", "", String.class.getName(),
            ...
            if (correlationInstance.getContainer().instanceOf("process.AbstractCatchMessageEvent")) {
                ...
                addReportChange((String) correlationInstance.getContainer().get("name"), correlationInstance.getContainer().getType().getEClass().getName(),
                        correlationInstance.getContainer().getUuid(), Messages.correlationMigrationDescription, Messages.correlationProperty, IStatus.WARNING);
                model.delete(correlationInstance);
            } else {
                addReportChange((String) correlationInstance.getContainer().get("name"), correlationInstance.getContainer().getType().getEClass().getName(),
                        correlationInstance.getContainer().getUuid(), Messages.correlationMigrationDescription, Messages.messagesProperty, IStatus.WARNING);
