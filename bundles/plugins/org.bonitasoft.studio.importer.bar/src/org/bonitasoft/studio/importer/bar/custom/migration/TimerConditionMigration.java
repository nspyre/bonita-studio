        ...
        for (Instance timer : model.getAllInstances("process.AbstractTimerEvent")) {
            ...
            if (timerCondition != null) {
                ...
                if (ExpressionConstants.SCRIPT_TYPE.equals(expressionType)) {
                    ...
                    addReportChange((String) timer.get("name"), timer.getType().getEClass().getName(), uuid, Messages.timerConditionMigrationDescription,
                            Messages.timerConditionProperty, IStatus.WARNING);
                } else {
                    addReportChange((String) timer.get("name"), timer.getType().getEClass().getName(), uuid, Messages.timerConditionMigrationDescription,
                            Messages.timerConditionProperty, IStatus.OK);
                }
            } else {
                expression = StringToExpressionConverter
                        .createExpressionInstance(model, "", "", Long.class.getName(), ExpressionConstants.CONSTANT_TYPE, false);
            }
