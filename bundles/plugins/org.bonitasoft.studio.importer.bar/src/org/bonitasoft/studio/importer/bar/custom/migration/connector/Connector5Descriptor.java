...
public class Connector5Descriptor {
    ...
    public Connector5Descriptor(Instance connectorInstance) {
        ...
        for (Instance output : outputMapping) {
            try {
                ...
                BonitaStudioLog.warning("The connector " + connectorId + "/" + name + " doesn't provide the expected feature for outputs.",
    ...
    private void createOperation(Model model, Instance connectorInstance,
        ...
        if (originalVariable != null) {
            ...
            leftOperand = StringToExpressionConverter.createExpressionInstanceWithDependency(model, variableToSet, variableToSet, returnType,
                    ExpressionConstants.VARIABLE_TYPE, true, originalVariable);
        } else {
            leftOperand = StringToExpressionConverter.createExpressionInstance(model, variableToSet, variableToSet, returnType,
                    ExpressionConstants.VARIABLE_TYPE, true);
        }
    ...
    private Instance getConnectorConfiguration(Model model, StringToExpressionConverter converter) {
        ...
        for (Entry<String, Object> input : allInput.entrySet()) {
            ...
            if (parameterKeyFor != null) {
                ...
                parameter.set(
                        "expression",
                        getParameterExpressionFor(model, parameterKeyFor, converter,
                                definitionMapper.transformParameterValue(parameterKeyFor, input.getValue(), inputs), getReturnType(parameterKeyFor)));
