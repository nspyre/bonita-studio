...
public class LoopConditionAndMultiInstanciationMigration extends
    ...
    public void migrateBefore(Model model, Metamodel metamodel)
        ...
        for (Instance activity : model.getAllInstances("process.Activity")) {
            ...
            if (multinsantiationInstance != null) {
                ...
                addReportChange((String) activity.get("name"), activity.getType().getEClass().getName(), activity.getUuid(),
                        Messages.removeMultiInstantiationMigrationDescription, Messages.iterationProperty, IStatus.ERROR);
    ...
    private void setLoopMaximum(Instance activity, Model model) {
        ...
        if (loopMaximums.containsKey(activity.getUuid())) {
            ...
            addReportChange((String) activity.get("name"), activity.getType().getEClass().getName(), activity.getUuid(),
                    Messages.loopMaximumScriptMigrationDescription, Messages.iterationProperty, ExpressionConstants.SCRIPT_TYPE.equals(expression.get("type"))
    ...
    private void setLoopCondition(Instance activity, Model model) {
        ...
        if (loopConditions.containsKey(activity.getUuid())) {
            ...
            addReportChange((String) activity.get("name"), activity.getType().getEClass().getName(), activity.getUuid(),
                    Messages.loopConditionScriptMigrationDescription, Messages.iterationProperty,
    ...
    private void setIerationType(Instance activity, Model model) {
        switch (iterationTypes.get(activity.getUuid())) {
            case NONE:
                activity.set("isLoop", false);
                activity.set("isMultiInstance", false);
                break;
            case LOOP:
                activity.set("isLoop", true);
                activity.set("isMultiInstance", false);
                break;
            case MULTI:
                activity.set("isLoop", false);
                activity.set("isMultiInstance", true);
                activity.set("multiInstantiation", model.newInstance("process.MultiInstantiation"));
                break;
            default:
                break;
        }
