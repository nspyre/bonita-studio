    ...
    private void setImgPath(final Model model, final Instance widget) {
        ...
        if (imgPaths.containsKey(widget.getUuid())) {
            ...
            if (widget.get("isADocument")) {
                ...
                addReportChange((String) widget.get("name"), widget.getEClass().getName(), widget.getUuid(), Messages.imgPathMigrationDescription,
    ...
    private void createDocuments(final Model model) {
        for (final Instance attachmentData : attachmentDatas.keySet()) {
            ...
            if (process.instanceOf("process.Pool")) {
                ...
                addReportChange((String) process.get("name"), process.getEClass().getName(), process.getUuid(),
                        Messages.bind(Messages.attachementDataRemovedFromMessage, attachmentData.get("name")), Messages.messagesProperty, IStatus.ERROR);
            }
