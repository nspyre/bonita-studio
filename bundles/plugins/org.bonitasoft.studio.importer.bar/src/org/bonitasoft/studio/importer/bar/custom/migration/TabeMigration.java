...
public class TabeMigration extends ReportCustomMigration {
                ...
                if (!row.isEmpty()) {
                    ...
                    for (String value : row) {
                        ...
                        if (ExpressionConstants.VARIABLE_TYPE.equals(expression.get("type"))) {
                            expression.set("returnType",
                                    StringToExpressionConverter.getDataReturnType(((List<Instance>) expression.get("referencedElements")).get(0)));
    ...
    private void setMinNumberOfRows(Instance widget, Model model) {
        ...
        if (minNumberOfRows.containsKey(widget.getUuid())) {
            ...
            addReportChange((String) widget.get("name"), widget.getEClass().getName(), widget.getUuid(), Messages.minNumberOfRowMigrationDescription,
    ...
    private void setMaxNumberOfRows(Instance widget, Model model) {
        ...
        if (maxNumberOfRows.containsKey(widget.getUuid())) {
            ...
            addReportChange((String) widget.get("name"), widget.getEClass().getName(), widget.getUuid(), Messages.maxNumberOfRowMigrationDescription,
                    Messages.dataProperty, StringToExpressionConverter.getStatusForExpression(expression));
        } else {
            expression = StringToExpressionConverter.createExpressionInstance(model, "", "", Integer.class.getName(), ExpressionConstants.CONSTANT_TYPE, true);
        }
    ...
    private void setMinNumberOfColumns(Instance widget, Model model) {
        ...
        if (minNumberOfColumns.containsKey(widget.getUuid())) {
            ...
            addReportChange((String) widget.get("name"), widget.getEClass().getName(), widget.getUuid(), Messages.minNumberOfColumnMigrationDescription,
                    Messages.dataProperty, StringToExpressionConverter.getStatusForExpression(expression));
        } else {
            expression = StringToExpressionConverter.createExpressionInstance(model, "", "", Integer.class.getName(), ExpressionConstants.CONSTANT_TYPE, true);
        }
    ...
    private void setMaxNumberOfColumns(Instance widget, Model model) {
        ...
        if (maxNumberOfColumns.containsKey(widget.getUuid())) {
            ...
            addReportChange((String) widget.get("name"), widget.getEClass().getName(), widget.getUuid(), Messages.maxNumberOfColumnMigrationDescription,
                    Messages.dataProperty, StringToExpressionConverter.getStatusForExpression(expression));
        } else {
            expression = StringToExpressionConverter.createExpressionInstance(model, "", "", Integer.class.getName(), ExpressionConstants.CONSTANT_TYPE, true);
        }
    ...
    private void setMaxRowForPaginations(Instance widget, Model model) {
        ...
        if (maxRowForPaginations.containsKey(widget.getUuid())) {
            ...
            addReportChange((String) widget.get("name"), widget.getEClass().getName(), widget.getUuid(), Messages.maxRowForPaginationMigrationDescription,
                    Messages.dataProperty, StringToExpressionConverter.getStatusForExpression(expression));
        } else {
            expression = StringToExpressionConverter.createExpressionInstance(model, "", "", Integer.class.getName(), ExpressionConstants.CONSTANT_TYPE, true);
        }
    ...
    private void setColumnForInitialIndexes(Instance widget, Model model) {
        ...
        if (columnForInitialIndexes.containsKey(widget.getUuid())) {
            ...
            addReportChange((String) widget.get("name"), widget.getEClass().getName(), widget.getUuid(), Messages.columnForInitialIndexMigrationDescription,
                    Messages.dataProperty, StringToExpressionConverter.getStatusForExpression(expression));
        } else {
            expression = StringToExpressionConverter.createExpressionInstance(model, "", "", Integer.class.getName(), ExpressionConstants.CONSTANT_TYPE, true);
        }
    ...
    private void setSelectedValues(Instance widget, Model model) {
        ...
        if (selectedValues.containsKey(widget.getUuid())) {
            ...
            addReportChange((String) widget.get("name"), widget.getEClass().getName(), widget.getUuid(), Messages.selectedValuesMigrationDescription,
                    Messages.dataProperty, StringToExpressionConverter.getStatusForExpression(expression));
        } else {
    ...
    private void setCells(Instance widget, Model model) {
        ...
        if (cells.containsKey(widget.getUuid())) {
            ...
            addReportChange((String) widget.get("name"), widget.getEClass().getName(), widget.getUuid(), Messages.cellExpressionMigrationDescription,
                    Messages.dataProperty, IStatus.WARNING);
    ...
    private void setVerticalHeaders(Instance widget, Model model) {
        ...
        if (verticalHeaders.containsKey(widget.getUuid())) {
            ...
            addReportChange((String) widget.get("name"), widget.getEClass().getName(), widget.getUuid(), Messages.verticalHeaderMigrationDescription,
                    Messages.dataProperty, StringToExpressionConverter.getStatusForExpression(expression));
        } else {
            expression = StringToExpressionConverter.createExpressionInstance(model, "", "", List.class.getName(), ExpressionConstants.CONSTANT_TYPE, true);
        }
    ...
    private void setHorizontalHeaders(Instance widget, Model model) {
        ...
        if (horizontalHeaders.containsKey(widget.getUuid())) {
            ...
            addReportChange((String) widget.get("name"), widget.getEClass().getName(), widget.getUuid(), Messages.horizontalHeaderMigrationDescription,
                    Messages.dataProperty, StringToExpressionConverter.getStatusForExpression(expression));
        } else {
            expression = StringToExpressionConverter.createExpressionInstance(model, "", "", List.class.getName(), ExpressionConstants.CONSTANT_TYPE, true);
        }
