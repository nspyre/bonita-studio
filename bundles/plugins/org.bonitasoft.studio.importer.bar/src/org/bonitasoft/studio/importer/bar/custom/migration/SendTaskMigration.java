            ...
            if (events.size() > 1) {
                ...
                for (int i = 0; i < events.size(); i++) {
                    if (i != 0) {
                        ...
                        addReportChange((String) sendTask.get("name"), events.get(i).getType().getEClass().getName(), sendTask.getUuid(),
                                Messages.bind(Messages.removeMessageEventFromSendTaskDescription, events.get(i).get("name")), Messages.messagesProperty,
                                IStatus.ERROR);
                    }
                }
