...
public class PageFlowMigration extends ReportCustomMigration {
    ...
    private void setPageFlowTransitionCondition(final Instance pageFlowTransition, final Model model) {
        ...
        if (pageFlowTransitionConditions.containsKey(pageFlowTransition.getUuid())) {
            ...
            addReportChange((String) pageFlowTransition.getContainer().get("name"), pageFlowTransition.getType().getEClass().getName(),
                    pageFlowTransition.getUuid(), Messages.pageFlowTransitionMigrationDescription, propertyName,
                    ExpressionConstants.SCRIPT_TYPE.equals(expression.get("type")) ? IStatus.WARNING : IStatus.OK);
        } else {
    ...
    private void setRecapRedirectionURL(final Instance pageFlow, final Model model) {
        ...
        if (recapRedirectionUrls.containsKey(pageFlow.getUuid())) {
            ...
            addReportChange((String) pageFlow.get("name"), pageFlow.getType().getEClass().getName(), pageFlow.getUuid(),
                    Messages.redirectionUrlMigrationDescription, Messages.recapPageflowProperty, ExpressionConstants.SCRIPT_TYPE.equals(expression.get("type"))
    ...
    private void setViewRedirectionURL(final Instance pageFlow, final Model model) {
        ...
        if (viewRedirectionUrls.containsKey(pageFlow.getUuid())) {
            ...
            addReportChange((String) pageFlow.get("name"), pageFlow.getType().getEClass().getName(), pageFlow.getUuid(),
                    Messages.redirectionUrlMigrationDescription, Messages.viewPageflowProperty, ExpressionConstants.SCRIPT_TYPE.equals(expression.get("type"))
                            ? IStatus.WARNING : IStatus.OK);
        } else {
            expression = StringToExpressionConverter.createExpressionInstance(model, "", "", String.class.getName(), ExpressionConstants.CONSTANT_TYPE, true);
        }
    ...
    private void setRedirectionActions(final Instance pageFlow, final Model model) {
        if (entryRedirectionActions.containsKey(pageFlow.getUuid())) {
            ...
            for (final Instance operation : entryRedirectionActions.get(pageFlow.getUuid())) {
                ...
                addReportChange((String) pageFlow.get("name"), pageFlow.getType().getEClass().getName(), pageFlow.getUuid(),
                        Messages.redirectionActionMigrationDescription, Messages.entryPageflowProperty,
                        ExpressionConstants.SCRIPT_TYPE.equals(((Instance) operation.get("rightOperand")).get("type")) ? IStatus.WARNING : IStatus.OK);
            }
        }
    }
    ...
    private void setEntryRedirectionURL(final Instance pageFlow, final Model model) {
        ...
        if (entryRedirectionUrls.containsKey(pageFlow.getUuid())) {
            ...
            addReportChange((String) pageFlow.get("name"), pageFlow.getType().getEClass().getName(), pageFlow.getUuid(),
                    Messages.redirectionUrlMigrationDescription, Messages.entryPageflowProperty, ExpressionConstants.SCRIPT_TYPE.equals(expression.get("type"))
                            ? IStatus.WARNING : IStatus.OK);
        } else {
            expression = StringToExpressionConverter.createExpressionInstance(model, "", "", String.class.getName(), ExpressionConstants.CONSTANT_TYPE, true);
        }
    ...
    private void setConfirmationMessage(final Instance pageFlow, final Model model) {
        ...
        if (confirmationMessages.containsKey(pageFlow.getUuid())) {
            ...
            addReportChange((String) pageFlow.get("name"), pageFlow.getType().getEClass().getName(), pageFlow.getUuid(),
                    Messages.confirmationMessageMigrationDescription, Messages.confirmationProperty,
                    ExpressionConstants.SCRIPT_TYPE.equals(expression.get("type")) ? IStatus.WARNING : IStatus.OK);
        } else {
            expression = StringToExpressionConverter.createExpressionInstance(model, "", "", String.class.getName(), ExpressionConstants.CONSTANT_TYPE, true);
        }
