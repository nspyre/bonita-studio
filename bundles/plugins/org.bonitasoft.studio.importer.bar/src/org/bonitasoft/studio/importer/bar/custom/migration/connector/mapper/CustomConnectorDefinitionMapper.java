    ...
    public boolean appliesTo(String legacyConnectorId) {
        ...
        for (ConnectorDefinition def : connectorDefStore.getDefinitions()) {
            if (legacyConnectorId.equals(def.getId())) {
                this.definition = def;
                ConnectorDefFileStore fileStore = connectorDefStore.getChild(NamingUtils.toConnectorDefinitionFilename(this.definition.getId(),
                        this.definition.getVersion(), true));
