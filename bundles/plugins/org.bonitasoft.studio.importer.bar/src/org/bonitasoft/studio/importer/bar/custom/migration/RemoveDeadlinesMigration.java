    ...
    public void migrateBefore(Model model, Metamodel metamodel)
        ...
        for (Instance deadline : model.getAllInstances("process.Deadline")) {
            ...
            if (container != null && (container.instanceOf("process.Task") || container.instanceOf("process.CallActivity"))) {
                ...
                addReportChange((String) connector.get("name"), "Deadline", deadline.getContainer().getUuid(),
                        Messages.deadlinesToNonInterruptingEventMigrationDescription, Messages.connectorProperty, IStatus.ERROR);
            } else {
                addReportChange((String) connector.get("name"), "Deadline", deadline.getContainer().getUuid(), Messages.removeDeadlinesMigrationDescription,
                        Messages.connectorProperty, IStatus.ERROR);
            }
