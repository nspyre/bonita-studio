        ...
        for (Instance sequenceFlow : model.getAllInstances("process.SequenceFlow")) {
            ...
            if (condition != null) {
                ...
                if (ExpressionConstants.SCRIPT_TYPE.equals(expressionType)) {
                    ...
                    addReportChange((String) sequenceFlow.get("name"), ProcessPackage.Literals.SEQUENCE_FLOW.getName(), uuid,
                            Messages.sequenceFlowConditionMigrationDescription, Messages.sequenceFlowConditionProperty, IStatus.WARNING);
                } else {
                    addReportChange((String) sequenceFlow.get("name"), ProcessPackage.Literals.SEQUENCE_FLOW.getName(), uuid,
                            Messages.sequenceFlowConditionMigrationDescription, Messages.sequenceFlowConditionProperty, IStatus.OK);
                }
            } else {
                expression = StringToExpressionConverter.createExpressionInstance(model, "", "", Boolean.class.getName(), ExpressionConstants.CONSTANT_TYPE,
                        true);
            }
