...
public class RemoveRowCommand extends AbstractTransactionalCommand {
    ...
    protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info) throws ExecutionException {
        ...
        for (Widget widget : getChildren()) {
            ...
            if (oldLine == row) {//remove element in line
                ...
                SetRequest setRequest = new SetRequest(getEditingDomain(), widget.getWidgetLayoutInfo(), FormPackage.eINSTANCE.getWidgetLayoutInfo_Line(),
                        oldLine - 1);
    ...
    protected void decrementRowEditPart() {
        SetRequest setRequest = new SetRequest(getEditingDomain(), semanticElement, FormPackage.eINSTANCE.getForm_NLine(),
                ((Form) semanticElement).getNLine() - 1);
        cc.add(new SetValueCommand(setRequest));
    }
    ...
    protected void decrementSpan(Widget widget) {
        SetRequest setRequest = new SetRequest(getEditingDomain(), widget.getWidgetLayoutInfo(), FormPackage.eINSTANCE.getWidgetLayoutInfo_VerticalSpan(),
                widget.getWidgetLayoutInfo().getVerticalSpan() - 1);
        cc.add(new SetValueCommand(setRequest));
    }
}
