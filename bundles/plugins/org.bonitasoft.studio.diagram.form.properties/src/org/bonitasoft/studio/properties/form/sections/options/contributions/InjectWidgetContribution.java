...
public class InjectWidgetContribution extends AbstractPropertySectionContribution {
    ...
    public void createControl(final Composite composite, final TabbedPropertySheetWidgetFactory widgetFactory,
        ...
        expressionViewer.addFilter(new AvailableExpressionTypeFilter(new String[] { ExpressionConstants.PARAMETER_TYPE, ExpressionConstants.VARIABLE_TYPE,
                ExpressionConstants.SCRIPT_TYPE, ExpressionConstants.CONSTANT_TYPE, ExpressionConstants.FORM_FIELD_TYPE, ExpressionConstants.DOCUMENT_TYPE }));
        //bind
        if (context != null) {
            context.dispose();
        }
        if (expressionViewer != null && !expressionViewer.getControl().isDisposed()) {
            ...
            context.bindValue(widgetValuePropertySelection.observe(checkbox),
                    EMFEditProperties.value(editingDomain, FormPackage.Literals.WIDGET__INJECT_WIDGET_CONDITION).observe(getEObject()));
            context.bindValue(WidgetProperties.enabled().observe(expressionViewer.getControl()),
                    EMFEditProperties.value(editingDomain, FormPackage.Literals.WIDGET__INJECT_WIDGET_CONDITION).observe(getEObject()),
                    never,
                    null);
        }
