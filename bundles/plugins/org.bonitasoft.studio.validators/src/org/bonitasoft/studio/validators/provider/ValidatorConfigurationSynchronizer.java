...
public class ValidatorConfigurationSynchronizer implements IConfigurationSynchronizer {
    ...
    private void updateFragment(Configuration configuration, FragmentContainer validatorContainer, CompoundCommand cc, EditingDomain editingDomain) {
        final ValidatorDescriptorRepositoryStore store = (ValidatorDescriptorRepositoryStore) RepositoryManager.getInstance().getRepositoryStore(
                ValidatorDescriptorRepositoryStore.class);
        ValidatorDescriptorFileStore file = (ValidatorDescriptorFileStore) store.getChild(validatorContainer.getId() + "."
                ...
                for (Fragment fragment : validatorContainer.getFragments()) {
                    if (!descriptor.getDependencies().contains(fragment.getValue()) && !validatorJar.equals(fragment.getValue())) { //Remove unused dependencies
                        cc.append(RemoveCommand
                                .create(editingDomain, validatorContainer, ConfigurationPackage.Literals.FRAGMENT_CONTAINER__FRAGMENTS, fragment));
                    }
                }
                ...
                for (String dep : descriptor.getDependencies()) {
                    ...
                    if (!depExists) {
                        ...
                        cc.append(AddCommand
                                .create(editingDomain, validatorContainer, ConfigurationPackage.Literals.FRAGMENT_CONTAINER__FRAGMENTS, newFragment));
                    }
                }
            }
        }
