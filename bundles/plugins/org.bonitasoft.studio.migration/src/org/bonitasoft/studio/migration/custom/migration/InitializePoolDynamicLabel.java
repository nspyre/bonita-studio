 ...
 *
 */
public class InitializePoolDynamicLabel extends CustomMigration {
	...
	public void migrateAfter(final Model model, final Metamodel metamodel)	throws MigrationException {
		...
		for(Instance pool : model.getAllInstances("process.Pool")){
				...
				Instance newLabelExpr = StringToExpressionConverter.createEmptyExpressionInstance(model, true);
				pool.set("dynamicLabel", newLabelExpr);
			}
	}
