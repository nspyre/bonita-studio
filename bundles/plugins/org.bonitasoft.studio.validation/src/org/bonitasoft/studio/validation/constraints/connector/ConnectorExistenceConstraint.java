...
public class ConnectorExistenceConstraint extends AbstractLiveValidationMarkerConstraint {
    ...
    protected IStatus performBatchValidation(IValidationContext context) {
        ...
        if (def != null) {
            ...
            if (isConnector) {
                return context.createFailureStatus(Messages.bind(Messages.Validation_noConnectorDefFound, connector.getName(), connector.getDefinitionId()
                        + "--" + connector.getDefinitionVersion()));
            } else {
                return context.createFailureStatus(Messages.bind(Messages.Validation_noActorFilterDefFound, connector.getName(), connector.getDefinitionId()
                        + "--" + connector.getDefinitionVersion()));
            }
