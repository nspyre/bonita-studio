    ...
    protected IStatus performBatchValidation(IValidationContext context) {
        ...
        if (def != null) {
            ...
            if (!fStore.isReadOnly()) {
                ...
                if (!inputStatus.isOK()) {
                    if (isConnector) {
                        return context.createFailureStatus(Messages.bind(Messages.Validation_InconsistentConnectorDefAndConfigurationInput, new Object[] {
                                connector.getName(), connector.getDefinitionId() + "--" + connector.getDefinitionVersion(), inputStatus.getMessage() }));
                    } else {
                        return context.createFailureStatus(Messages.bind(Messages.Validation_InconsistentActorDefAndConfigurationInput, new Object[] {
                                connector.getName(), connector.getDefinitionId() + "--" + connector.getDefinitionVersion(), inputStatus.getMessage() }));
                    }
                }
            }
