...
// // AbstractProcess modelSubProcess = ModelHelper.findProcess(ModelHelper.getMainProcess(process),
// subProcEventDef.getName());//ModelHelper.findProcess(ProcessRepository.getInstance().getAllProcesses(), subProcEventDef.get);
// // /*And if we find one and its name is not the same as the first*/
// // String processName = process.getName();
// // if(modelSubProcess != null){
// // String subProcessName = modelSubProcess.getName();
...
//
//
// BusinessArchive bar = exportAsBarFileCommand.createBusinessArchive((AbstractProcess)process,deploymentMode.equals(BonitaPreferenceConstants.ALL_IN_BAR),
// options, additionalResources,excludedObject);
...
// List<ProcessDeploymentInfo> processes = BOSEngineManager.getInstance().getProcessAPI().getProcesses(0, (int)
// BOSEngineManager.getInstance().getProcessAPI().getNumberOfProcesses(), ProcessDefinitionCriterion.DEFAULT) ;
