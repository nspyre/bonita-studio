...
public class SubProcessPropertySectionContribution implements IExtensibleGridPropertySectionContribution {
     ...
     * org.eclipse.ui.views.properties.tabbed.TabbedPropertySheetWidgetFactory, org.bonitasoft.studio.properties.sections.general.ExtensibleGridPropertySection)
     */
    @Override
    public void createControl(final Composite composite, TabbedPropertySheetWidgetFactory widgetFactory,
        ...
        if (nameExp == null) {
            ...
            editingDomain.getCommandStack().execute(
                    SetCommand.create(editingDomain, subprocess, ProcessPackage.Literals.CALL_ACTIVITY__CALLED_ACTIVITY_NAME, nameExp));
        }

        dbc.bindValue(ViewersObservables.observeSingleSelection(nameViewer),
                EMFEditObservables.observeValue(editingDomain, subprocess, ProcessPackage.Literals.CALL_ACTIVITY__CALLED_ACTIVITY_NAME));

        widgetFactory.createLabel(composite, Messages.version);

        versionViewer = new ExpressionViewer(composite, SWT.BORDER, widgetFactory, editingDomain,
        ...
        versionViewer.addFilter(new AvailableExpressionTypeFilter(new String[] { ExpressionConstants.CONSTANT_TYPE, ExpressionConstants.VARIABLE_TYPE,
                ExpressionConstants.SCRIPT_TYPE, ExpressionConstants.PARAMETER_TYPE }));
        ...
        if (versionExp == null) {
            ...
            editingDomain.getCommandStack().execute(
                    SetCommand.create(editingDomain, subprocess, ProcessPackage.Literals.CALL_ACTIVITY__CALLED_ACTIVITY_VERSION, versionExp));
        }

        dbc.bindValue(ViewersObservables.observeSingleSelection(versionViewer),
                EMFEditObservables.observeValue(editingDomain, subprocess, ProcessPackage.Literals.CALL_ACTIVITY__CALLED_ACTIVITY_VERSION));
    }
