    ...
    public Object execute(ExecutionEvent event) throws ExecutionException {
        if (isEnabled()) {
            ...
            if (wizardDialog.open() == IDialogConstants.OK_ID) {
                ...
                SimulationWithMonitorRunner runnable = new SimulationWithMonitorRunner(wizard.getSelectedProcess(), wizard.getPath(),
                        wizard.getLoadProfileId(), wizard.getTimespan());
                try {
                        ...
                        if (!fileStore.fetchInfo().isDirectory() && fileStore.fetchInfo().exists()) {
                            ...
                            try {
                                ...
                                IWebBrowser browser = PlatformUI.getWorkbench().getBrowserSupport()
                                        .createBrowser(IWorkbenchBrowserSupport.AS_EDITOR, "", fileStore.getName(), ""); //$NON-NLS-1$
