...
public class ConvertBPMNTypeCommand extends AbstractTransactionalCommand {
    ...
    public ConvertBPMNTypeCommand(final TransactionalEditingDomain domain, final EClass targetEClass, final GraphicalEditPart node,
            final ElementTypeResolver elementTypeResolver) {
    ...
    protected CommandResult doExecuteWithResult(final IProgressMonitor monitor, final IAdaptable adaptable) throws ExecutionException {
        ...
        if (ProcessPackage.eINSTANCE.getActivity().isSuperTypeOf(targetEClass)) {
            final ChangePropertyValueRequest fillReq = new ChangePropertyValueRequest("Fill Color", Properties.ID_FILLCOLOR, ((FillStyle) node
                    .getNotationView()
                    .getStyle(NotationPackage.eINSTANCE.getFillStyle())).getFillColor());
            targetEditPart.performRequest(fillReq);
            final ChangePropertyValueRequest lineReq = new ChangePropertyValueRequest("Line Color", Properties.ID_LINECOLOR, ((LineStyle) node
                    .getNotationView()
            ...
            for (final BoundaryEvent boundaryEvent : ((Activity) targetElement).getBoundaryIntermediateEvents()) {
                ...
                if (boundaryEp != null) {
                    for (int i = 0; i < boundaryEvent.getOutgoing().size(); i++) {
                        ...
                        final GraphicalEditPart boundaryTargetEP = (GraphicalEditPart) parentEditPart.findEditPart(parentEditPart.getRoot(),
                                outgoing.getTarget());

                        if (boundaryTargetEP != null) {

                            final ConnectionEditPart ep = (ConnectionEditPart) GMFTools.findEditPart((EditPart) parentEditPart.getRoot().getChildren().get(0),
                                    outgoing);
                            if (ep != null) {
                                try {
