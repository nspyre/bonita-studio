                        ...
                        if (fileName != null) {
                            ...
                            if (IStatus.OK == status.getSeverity()) {
                                Display.getDefault().syncExec(new Runnable() {
                                    ...
                                    @Override
                                    public void run() {
                                        MessageDialog.openInformation(Display.getDefault().getActiveShell(), Messages.importSuccessfulTitle,
                                                Messages.importSuccessfulMsg);
                                    }
                                });
                            } else {
                                Display.getDefault().syncExec(new Runnable() {
                                    ...
                                    @Override
                                    public void run() {
                                        MessageDialog.openError(Display.getDefault().getActiveShell(), Messages.importFailedTitle,
                                                Messages.bind(Messages.importFailedMsg, status.getMessage()));
                                    }
                                });
                            }
                        }
