...
public class RefactorActorMappingsOperation implements IRunnableWithProgress {
    ...
    public void run(IProgressMonitor monitor) throws InvocationTargetException,
        ...
        ProcessConfigurationRepositoryStore confStore = (ProcessConfigurationRepositoryStore) RepositoryManager.getInstance().getRepositoryStore(
        ...
        for (DiffElement difference : differences) {
            ...
            List<ModelElementChangeLeftTarget> newElementChange = ModelHelper.getAllItemsOfType(difference,
                    DiffPackage.Literals.MODEL_ELEMENT_CHANGE_LEFT_TARGET);
            List<ModelElementChangeRightTarget> oldElementChange = ModelHelper.getAllItemsOfType(difference,
    ...
    protected void refactorGroup(org.bonitasoft.studio.actors.model.organization.Membership oldMembership,
            org.bonitasoft.studio.actors.model.organization.Membership newMembership, List<ActorMappingsType> actorMappings) {
        for (ActorMappingsType ac : actorMappings) {
            if (!ac.getActorMapping().isEmpty()) {
                Groups groups = ac.getActorMapping().get(0).getGroups();
                if (groups != null) {
