    ...
    public void changeProcessNameAndVersion(final AbstractProcess process, final String name, final String version) {
        ...
        if (process instanceof MainProcess) {
            editingDomain.getCommandStack().execute(
                    SetCommand.create(editingDomain, process, ProcessPackage.Literals.ABSTRACT_PROCESS__AUTHOR, System.getProperty("user.name", "Unknown")));
            editingDomain.getCommandStack().execute(
                    SetCommand.create(editingDomain, process, ProcessPackage.Literals.MAIN_PROCESS__CONFIG_ID, ConfigurationIdProvider
                            .getConfigurationIdProvider().getConfigurationId((MainProcess) process)));
        }
    }
                        ...
                        for (CallActivity sub : toUpdate) {
                            ...
                            if (subprocessName != null) {
                                ...
                                if (!subprocessName.equals(NamingUtils.convertToId(newPoolName))) {
                                    cc.append(SetCommand.create(domain, toModify, ProcessPackage.Literals.CALL_ACTIVITY__CALLED_ACTIVITY_NAME,
                                            createStringExpression(newPoolName)));
                                }
                                if (subprocessVersion != null && newVersion != null && !(newVersion.equals(oldVersion))) {
                                    cc.append(SetCommand.create(domain, toModify, ProcessPackage.Literals.CALL_ACTIVITY__CALLED_ACTIVITY_VERSION,
                                            createStringExpression(newVersion)));
                                }
