...
public class CustomReceiveTask2EditPart extends ReceiveTask2EditPart {
    ...
    protected void setBackgroundColor(Color color) {
        if (primaryShape != null) {
            ((CustomSVGFigure) primaryShape).setColor(
                    ColorRegistry.getInstance().getColor(((LineStyle) getNotationView().getStyle(NotationPackage.eINSTANCE.getLineStyle())).getLineColor()),
                    color);
        }
    }
    ...
    protected void setForegroundColor(Color color) {
        if (primaryShape != null) {
            ((CustomSVGFigure) primaryShape).setColor(color,
                    ColorRegistry.getInstance().getColor(((FillStyle) getNotationView().getStyle(NotationPackage.eINSTANCE.getFillStyle())).getFillColor()));
        }
    }
