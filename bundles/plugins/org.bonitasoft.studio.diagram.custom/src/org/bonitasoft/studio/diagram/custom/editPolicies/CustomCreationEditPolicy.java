...
public class CustomCreationEditPolicy extends CreationEditPolicy {
    ...
    private Command getUnspecifiedTypeCreateCommand(
        ...
        for (Iterator<?> iter = requestElementTypes.iterator(); iter.hasNext();) {
            ...
            if (createRequest != null) {
                ...
                if (((target instanceof CustomLaneCompartmentEditPart || target instanceof CustomLaneNameEditPart) && requestElementTypes
                        .contains(ProcessElementTypes.Lane_3007))) {
        ...
        if (createCmds.isEmpty()) {
            ...
            CustomCreateOrSelectElementCommand selectAndCreateViewCmd = new CustomCreateOrSelectElementCommand(
                    DiagramUIMessages.CreateCommand_Label, Display.getCurrent().getActiveShell(), validTypes, LayerManager.Helper.find(getHost()).getLayer(
    ...
    private Command editTimerEventValue(IGraphicalEditPart timerPart) {
        ...
        if (!(scriptType.equals(StartTimerScriptType.GROOVY) || scriptType.equals(StartTimerScriptType.CONSTANT))) {
            cc.add(new EMFtoGEFCommandWrapper(new SetCommand(timerPart.getEditingDomain(), timer.getCondition(),
                    ExpressionPackage.Literals.EXPRESSION__CONTENT, ""), timerPart.getEditingDomain()));
            cc.add(new EMFtoGEFCommandWrapper(new SetCommand(timerPart.getEditingDomain(), timer.getCondition(), ExpressionPackage.Literals.EXPRESSION__NAME,
                    ""), timerPart.getEditingDomain()));
            cc.add(new EMFtoGEFCommandWrapper(new SetCommand(timerPart.getEditingDomain(), timer, ProcessPackage.Literals.START_TIMER_EVENT__SCRIPT_TYPE,
                    StartTimerScriptType.GROOVY), timerPart.getEditingDomain()));
