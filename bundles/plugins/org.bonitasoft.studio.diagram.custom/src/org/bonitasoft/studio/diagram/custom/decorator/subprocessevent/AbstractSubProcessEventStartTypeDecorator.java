...
public abstract class AbstractSubProcessEventStartTypeDecorator extends AbstractBonitaDecorator {
    ...
    protected void activateDiagramEventBroker(EObject node, TransactionalEditingDomain domain,
        ...
        if (node instanceof SubProcessEvent) {
            if (getDecoratorTarget().getAdapter(EditPart.class) != null && getDecoratorTarget().getAdapter(EditPart.class) instanceof SubProcessEvent2EditPart) {
                ...
                if (v != null) {
                    DiagramEventBroker.getInstance(domain).addNotificationListener(v, NotationPackage.eINSTANCE.getDrawerStyle_Collapsed(),
                            notificationListener);
                }
            }
            DiagramEventBroker.getInstance(domain).addNotificationListener(((SubProcessEvent) node), ProcessPackage.eINSTANCE.getContainer_Elements(),
                    notificationListener);
    ...
    protected void deactivateDiagramEventBroker(IGraphicalEditPart part) {
        if (part instanceof SubProcessEvent2EditPart) {
            ...
            if (v != null) {
                DiagramEventBroker.getInstance(part.getEditingDomain()).removeNotificationListener(v, NotationPackage.eINSTANCE.getDrawerStyle_Collapsed(),
                        notificationListener);
            }
        }
        DiagramEventBroker.getInstance(part.getEditingDomain()).removeNotificationListener(part.resolveSemanticElement(),
